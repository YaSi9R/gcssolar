...
const Hero = () => {
  ...
  return (
    <section className="relative w-full overflow-hidden bg-white mb-10 md:mb-20 pt-[72px] lg:pt-[84px] h-[75vh] sm:h-[80vh] md:h-[100vh] flex items-center">
      {/* Background Image Slider */}
      <div className="absolute inset-0 w-full h-full z-0">
        <AnimatePresence mode="wait">
          <motion.div
            key={`bg-${current}`}
            initial={{ opacity: 0, scale: slide.video ? 1 : 1.1 }}
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0 }}
            transition={{ duration: 1 }}
            className="absolute top-[72px] lg:top-[84px] left-0 right-0 bottom-0 w-full h-full"
          >
            {slide.video ? (
              <video
                src={slide.video}
